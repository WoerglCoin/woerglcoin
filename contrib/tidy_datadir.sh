...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Woerglcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Woerglcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Woerglcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Woerglcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Woerglcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Woerglcoin 0.8 datadir."
    ;;
esac
